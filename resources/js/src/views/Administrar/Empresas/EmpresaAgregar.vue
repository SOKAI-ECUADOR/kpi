<template>
  <div class="vx-row">
    <div class="vx-col w-full ">
      <vx-card >
        <h4 class="mb-3">Agregar Empresa</h4>
        <div class="vx-row">
          ...
          <div class="vx-col sm:w-1/4 w-full mb-6">
            ...
            <vx-input-group>
                <vs-input class="w-full"  v-model="ruc" onkeypress="return enteros(event,this);" @blur="validarruc"  maxlength="13"/>
                <template slot="append">
                    <div class="append-text btn-addon">
                        <vs-button color="primary" type="filled" icon="search" @click="seachRuc(ruc)"></vs-button>              
                    </div>
                </template>
            </vx-input-group>
            <div v-show="error">
        ...
        <div class="vx-row">
          ...
          <div class="vx-col sm:w-1/3 w-full mb-6">
            ...
            <div v-show="error">
              <span class="text-danger" v-for="(err,index) in errortipoemision" :key="index" v-text="err"></span>
            </div>
          </div>
          <div class="vx-col sm:w-1/3 w-full mb-6">
            ...
            <div v-show="error">
              <span class="text-danger" v-for="(err,index) in errorambiente" :key="index" v-text="err"></span>
            </div>
          </div>
        ...
        <div class="vx-row">
          ...
          <div v-if="!$route.params.id" class="vx-col sm:w-1/2 w-full mb-6">
        ...
        <vs-collapse>
          <vs-collapse-item icon-pack="feather" icon-arrow="icon-chevrons-down">
            ...
            <div class="vx-row">
              ...
              <!--
                Dicho anteriormente los permisos, aqui lista los permisos mediante las listas de módulo igual al formato de usuarios
                el modulo revisa si no es permiso de empresa en si en el if de v-if="val.value!=1" por que empresa no puede dar permiso
                los datos de roles recupera mediante el json que se encuentra en la carpeta de storage
              -->
              <div class="vx-col sm:w-full w-full mb-6">
                  ...
                  <div slot="header" style="margin-left: 13px;padding: 16px 0;font-weight: bold;font-size: 22px;display: inline-flex;">
                      Salud
                      <vs-checkbox class="selectall" v-model="b_salud" @click="salud_p()"/>
                  </div>
                  <table class="w-full" style="margin: 0 20px;">
                    <tr>
                      <th class="font-semibold text-base text-left px-3 py-2" style="width:40%">Modulo</th>
                  ...
                  <table class="w-full" style="margin: 0 20px;">
                    ...
                    <tr v-for="(val, index) in items" :key="index" v-if="val.lugar==11">
                      <td class="px-3 py-2">{{ val.nombre }}</td>
                      <td class="px-3 py-2"><vs-checkbox v-model="val.ver"/></td>
                      <td class="px-3 py-2"><vs-checkbox v-if="val.value!=1" v-model="val.editar"/></td>
                      <td class="px-3 py-2"><vs-checkbox v-if="val.value!=1" v-model="val.crear"/></td>
...
export default {
  ...
  methods: {
    ...
    listarciudades(){
      setTimeout(() => {
        axios.get('/api/ciudades?id='+this.provincia).then( res => {
          ...
          if(this.addCiudadPorRuc){
            var ciud = this.ciudades.find(
              ciu => ciu.nombre === this.ciudadPorRuc
            );
            this.this.ciudad = ciud.id_ciudad;
            this.addCiudadPorRuc = false;
          }
        });
      }, 200);
    },
    ...
    guardar(){
      ...
      if(this.primerc==null || this.primerc=='null'){
        formData.append("compra", null);
      }else{
        formData.append("compra", this.primerc);
      }
      if(this.segundoc==null || this.segundoc=='null'){
        formData.append("migo", null);
      }else{
      ...
      axios.post("/api/agregarempresa",formData).then( ({data}) => {
        axios.post("/api/agregarempresaroles",{
          roles: this.items,
          id: data,
        }).then( () => {
          this.$vs.notify({
            title: "Empresa creada",
            text: "La empresa se creo exitosamente",
            color: "success"
          });
    ...
    validar(){
      ...
      this.errortipoemision = [];
      this.errorambiente = [];
      this.errorcategoria_producto=[];
      if(!this.tipoemision){
        this.errortipoemision.push("Ingrese el tipo");
        this.error = 1;
      }
      if(!this.ambiente){
        this.errorambiente.push("Ingrese el ambiente");
        this.error = 1;
      }
      if(!this.ruc){
        this.errorruc.push("Ruc obliatorio");
        this.error = 1;
        console.log("1");
      }
      ...
      if(!this.razon_social){
        this.errorrazon_social.push("Razón social obliatorio");
        this.error = 1;
        console.log("3");
      }
      if(!this.provincia){
      ...
      if(!this.ciudad){
        this.errorciudad.push("Ciudad obliatorio");
        this.error = 1;
        console.log("5");
      }
      if(!this.secuencial_factura){
        this.errorsecuencial_factura.push("Secuencial obliatorio");
        this.error = 1;
        console.log("8");
      }
      if(!this.secuencial_nota_credito){
        this.errorsecuencial_nota_credito.push("Secuencial obliatorio");
        this.error = 1;
        console.log("9");
      }
      if(!this.secuencial_nota_debito){
        this.errorsecuencial_nota_debito.push("Secuencial obliatorio");
        this.error = 1;
        console.log("10");
      }
      if(!this.secuencial_guia_remision){
        this.errorsecuencial_guia_remision.push("Secuencial obliatorio");
        this.error = 1;
        console.log("11");
      }
      if(!this.secuencial_retencion){
        this.errorsecuencial_retencion.push("Secuencial obliatorio");
        this.error = 1;
        console.log("12");
      }
      if(!this.secuencial_liquidacion_compra){
        this.errorsecuencial_liquidacion_compra.push("Secuencial obliatorio");
        this.error = 1;
        console.log("13");
      }
      if(!this.nombreusuario){
        this.errornombreusuario.push("Nombre obliatorio");
        this.error = 1;
        console.log("14");
      }
      if(!this.apellidousuario){
        this.errorapellidousuario.push("Apellido obliatorio");
        this.error = 1;
        console.log("15");
      }
      if(!this.emailusuario){
        this.erroremailusuario.push("Email obliatorio");
        this.error = 1;
        console.log("16");
      }
      if(!this.passusuario){
        this.errorpassusuario.push("Contraseña obliatorio");
        this.error = 1;
        console.log("17");
      }
      ...
      // else{
      //   var busqueda = this.categoria_producto.reduce((acc, persona) => {
      //     acc[persona.value] = ++acc[persona.value] || 0;
      //     return acc;
      //   }, {});

      //   var duplicados = this.categoria_producto.filter( (persona) => {
      //     return busqueda[persona.value];
      //   });
      //   if(duplicados.length>0){
      //     this.errorcategoria_producto.push("Hay Categorias Duplicadas");
      //     this.error = 1;
      //     console.log("20");
      //   }
      // }
    ...
    listarroles() {
        //lista los permisos de los roles y los guarda en la variable de data
      axios.get('/api/empresaroles').then( ({data}) => {
        this.items = data;
      });
    },
    //lista los permisos de contabilidad
    contabilidads(){
    ...
    //lista los permisos de facturacion
    facturacion(){
    ...
    //lista los permisos de inventario
    inventario(){
    ...
    //lista los permisos de importacion
    importacion(){
    ...
    //lista los permisos de produccion
    produccion(){
    ...
    //lista los permisos de calendario
    calendario(){
    ...
    //verificacion del correo de prueba manda un correo de prueba a la empres y revisa si funciona correctamente
    verificarcorreo(){
    ...
    //los telefonos se listan como arrays para poder guardar varios en el mismo campo
    remove_chip_correo (item) {
        this.telefono.splice(this.telefono.indexOf(item), 1)
    },
    //buscador de ruc en SRI para ingresar datos automaticmaente
            seachRuc(ruc) {
            axios
                ...
                .then(({ data }) => {
                    ...
                    if (data.clase_contribuyente == "Especial") {
                      this.contribuyente = 1;
                    } else {
                      this.contribuyente = 0;
                    }
                    this.direccion = data.direccion;
                    //asignar provincia y canton con direccion de ruc
                    var provin = this.provincias.find(
                        prov => prov.nombre === data.provincia
                    );
                    this.addCiudadPorRuc = true;
                    this.ciudadPorRuc = data.ciudad;
                    this.provincia = provin.id_provincia;
                    /*setTimeout(() => {
                        var ciud = this.ciudades.find(
                            ciu => ciu.nombre === data.ciudad
                        );
                        this.ciudad = ciud.id_ciudad;
                    }, 300);*/
                })
                .catch(err => {
                    console.log(err);
