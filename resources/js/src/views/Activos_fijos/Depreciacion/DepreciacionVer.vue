    ...
    <div id="invoice-page">
        <vx-card>
            ...
            <vs-divider position="left" class="flexy" >
                <h3>Activos Fijos</h3>
                <vs-switch vs-icon-on="check" color="success" class="ml-2" v-model="estado_activos" disabled vs-value="Todos" style="margin-top: 4px;" v-if="!tipo_activo && !grupo_activo">
                ...
                <vs-table hoverFlat :data="valoractivos" style="font-size: 12px;">
                    ...
                    <template slot-scope="{data}">
                        <vs-tr v-for="(tr, index) in data" :key="index">
                            ...
                            <vs-td :data="tr.fecha_activo">{{tr.fecha_activo}}</vs-td>
                            <vs-td :data="tr.dias_transcurridos">{{tr.dias_transcurridos}}</vs-td>
                            <vs-td :data="tr.tipo_activo">{{tr.tipo_activo}}</vs-td>
                            <vs-td :data="tr.grupo_activo">{{tr.grupo_activo}}</vs-td>
                            <vs-td style="text-align:right!important" :data="tr.valor_depreciacion">{{tr.valor_depreciacion}}</vs-td>
                            
                        </vs-tr>
                    </template>
                    </vs-table>
                </div>
                <div class="vx-col sm:w-full w-full mb-6 relative"  hidden>
...
export default {
        ...
        total_bien(){
            if(this.valoractivos.length>0){
                ...
                this.valoractivos.forEach(el => {
                    total += parseFloat(el.valor_depreciacion)
                });
                return total;
            }
        }
    ...
    methods:{
        listarDepreciacion(){
            let me = this;
            var url =
                "/api/depreciacion/abrir/" +
                this.$route.params.id;
                
            axios
                .get(url)
                .then(({data}) => {
                    me.fecha_inicio = data.cabecera.fecha_inicio;
                    me.fecha_fin = data.cabecera.fecha_fin;
                    me.grupo_activo = data.cabecera.id_grupo_activo_fijo;
                    me.tipo_activo = data.cabecera.id_tipo_activo_fijo;
                    if(data.cabecera.todos_activos==0){
                        me.estado_activos=false;
                    }else{
                         me.estado_activos=true;
                    }
                    me.valoractivos=data.detalle;   
                })
