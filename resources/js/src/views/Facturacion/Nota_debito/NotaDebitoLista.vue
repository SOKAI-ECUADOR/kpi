      ...
      <div class="flex flex-wrap justify-between items-center mb-3">
        ...
        <div class="flex flex-wrap items-center justify-between ag-grid-table-actions-right">
          ...
          <div class="dropdown-button-container" v-if="crearrol">
            <vs-button class="btnx" type="filled" to="/facturacion/nota-debito/agregar">Agregar</vs-button>
            <vs-dropdown>
            ...
            <vs-td class="whitespace-no-wrap text-center estilosacciones">
              <vs-dropdown vs-custom-content vs-trigger-click>
                  ...
                  <vs-dropdown-menu style="width:13em;">
                      ...
                      <vs-dropdown-item divider class="text-center" @click="ver(datos.id_nota_debito)" v-if="datos.respuesta == 'Enviado' && editarrol && datos.estadof != 0"><feather-icon icon="EyeIcon" svgClasses="w-5 h-5"></feather-icon> Visualizar</vs-dropdown-item>
                      <vs-dropdown-item class="text-center" @click="ver(datos.id_nota_debito)" v-if="datos.estadof == 0"><feather-icon icon="EyeIcon" svgClasses="w-5 h-5"></feather-icon> Visualizar</vs-dropdown-item>
                      <vs-dropdown-item divider class="text-center" @click="editar(datos.id_nota_debito)" v-if="datos.respuesta!='Enviado' && editarrol && datos.estadof != 0 && datos.contabilidad==null"><feather-icon icon="EditIcon" svgClasses="w-5 h-5"></feather-icon> Editar</vs-dropdown-item>
                      <vs-dropdown-item divider class="text-center" @click="eliminar(datos.id_nota_debito)" v-if="eliminarrol && datos.estadof != 0 && datos.contabilidad==null"><feather-icon icon="TrashIcon" svgClasses="w-5 h-5"></feather-icon> Cancelar</vs-dropdown-item>
                  </vs-dropdown-menu>
              </vs-dropdown>
              <feather-icon icon="SlidersIcon" class="cursor-pointer" v-if="datos.estadof>0 && datos.contabilidad==null" svgClasses="w-5 h-5 fill-current text-primary"  @click="Contabilidad(datos.id_nota_debito)"/>
                  <feather-icon icon="SlidersIcon" class="cursor-pointer" v-if="datos.estadof>0 && datos.contabilidad!==null" svgClasses="w-5 h-5 fill-current text-success"  @click="Contabilidad(datos.id_nota_debito)"/>
      ...
      <div class="vx-row">
        ...
        <div class="vx-col xs:w-10/12 sm:w-11/12">
                <div class="vx-row">
                    ...
                    <div class="vx-col sm:w-1/12 w-full mb-2" v-if="data.debe>0 && data.fecha_pago!==null">
                        <vs-button
                            type="filled"
        ...
        <div class="vx-col xs:w-10/12 sm:w-11/12">
                <div class="vx-row">
                    ...
                    <div class="vx-col sm:w-1/12 w-full mb-6" v-if="data.debe>0 && data.fecha_pago!==null">
                        <vs-button
                            type="filled"
...
export default {
  ...
  methods: {
    ...
    editar(id) {
      this.$router.push(`/facturacion/nota-debito/${id}/editar`);
    },
    ver(id) {
      this.$router.push(`/facturacion/nota-debito/${id}/ver`);
    },
    facturaenvio(dat) {
      this.$vs.notify({
        time:8000,
    ...
    acceptAlert(parameters) {
      ...
      axios.delete("/api/eliminardebito/" + parameters);
      //this.modal=false;
      this.$vs.notify({
        color: "warning",
    ...
    descargarxml(datos){
        window.open('/'+datos.id_empresa+'/vistaxml/nota_debito/'+datos.clave_acceso, '_top');
    },
    descargarpdfyxml(datos){
        window.open('/'+datos.id_empresa+'/vistapdfyxml/nota_debito/'+datos.clave_acceso, '_top');
    },
    enviocorreo(datos){
      this.$vs.notify({
          time: 3000,
    ...
    //methodos Asientos
    Contabilidad(id){
      axios.get('/api/notadebito/verasiento/' + id+
                        "?id_empresa=" +
                        this.usuario.id_empresa).then( ({data}) => {
                          ...
                          if(data.nota_credito_fact.contabilidad==1){
                              this.codigo="NDF-"+data.codigo_anterior;
                              this.contabilizado=data.nota_credito_fact.contabilidad;
                          }else{
                              this.codigo="NDF-"+data.codigo;
                              this.contabilizado=null;
                          }
                              this.concepto="Debito Facturacion "+serie1+"-"+serie2+"-"+documento+" Cliente: "+this.razon_social;
    ...
    crearasiento(id){
            ...
            axios.post("/api/nota_debito_fact/agregar/asiento",{
                cod_rol:id,
    ...
    crearasientoDetalle(id){
            axios.post("/api/nota_debito_fact/agregar/asiento_detalle",{
