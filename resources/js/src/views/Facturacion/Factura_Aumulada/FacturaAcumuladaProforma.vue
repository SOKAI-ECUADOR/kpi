    ...
    <div id="invoice-page">
        <vx-card>
                    ...
                    <vs-table hoverFlat :data="producto.lista_productos" style="font-size: 12px;">
                        ...
                        <template slot-scope="{ data }">
                            <vs-tr v-for="(tr, index) in data" :key="index" class="fila_lista">
                                ...
                                <vs-td style="width:70px!important;" v-else>
                                    <template v-if="tr.p_descuento==1">
                                         $ {{ tr.subtotal =  ((tr.cantidad * parseFloat(tr.precio).toFixed(2)) - tr.descuento).toFixed(2)}}
                                    </template>
                                    <template v-else>
                                         $ {{ tr.subtotal =  ((tr.cantidad * parseFloat(tr.precio).toFixed(2)) - ((tr.cantidad * parseFloat(tr.precio).toFixed(2) * tr.descuento)/100)).toFixed(2)}}
                                    </template>
                                </vs-td>
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="success" type="filled" :disabled="disabled" @click="verificarclaves()">GUARDAR</vs-button>
                <vs-button color="danger" type="filled" to="/facturacion/factura_acumulada">CANCELAR</vs-button>
            </div>
            <!-- Crear Cliente -->
            <vs-popup :title="modal.titulo" :active.sync="modal.abrir" class="modal-xl">
                ...
                <!-- <div class="con-exemple-prompt">
                    <div class="vx-row">
                        <div class="vx-col  sm:w-1/5 w-full relative mb-6" v-if="codigocliente">
                            <vs-input class="w-full" label="Código de cliente" v-model="codigocliente" disabled/>
                            <div v-show="error" v-if="!codigocliente">
                                <div v-for="err in errorcodigocliente" :key="err" v-text="err" class="text-danger"></div>
                            </div>
                        </div>
                        <div class="vx-col  sm:w-1/5 w-full relative mb-6" v-else>
                        ...
                        <div class="vx-col sm:w-1/2 sm:w-1/2 w-full relative mb-6">
                            <vs-input class="w-full" label="Nombre Completo" v-model="crear_cliente.nombre" @input="nombreaddicional(crear_cliente.nombre)"/>
                        ...
                        <div class="vx-col sm:w-1/4 sm:w-1/2 w-full relative mb-6">
                            ...
                            <vs-select class="selectExample w-full" placeholder="Seleccione el tipo" v-model="crear_cliente.tipo_identificacion" @change="cambio(crear_cliente.tipo_identificacion)">
                                <vs-select-item :key="index" :value="item.value" :text="item.text" v-for="(item, index) in tipo_identificacion_menu"/>
                            </vs-select>
...
export default {
    ...
    computed: {
        ...
        formulas(){
            ...
            if(this.usuario.id_empresa==55){
                ...
                this.producto.lista_productos.forEach(el => {
                    if (el.p_descuento == 1) {
                        subtotal += parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento;

                        if (el.total_ice) {subtotalice += parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento;}
                        if (el.total_ice) {valorice += el.total_ice * el.cantidad;}

                        if (el.iva == 2) {subtotal12 += (parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento) + (el.total_ice*el.cantidad);}
                        if (el.iva == 1) {subtotal0 += (parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento) + (el.total_ice*el.cantidad);}
                        if (el.iva == 3) {no_impuesto += (parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento) + (el.total_ice*el.cantidad);}
                        if (el.iva == 4) {exento += (parseFloat(el.precio).toFixed(2) * el.cantidad - el.descuento) + (el.total_ice*el.cantidad);}
                        if(isNaN(parseFloat(el.descuento))){
                            descuento += 0;
                        }else{
                            descuento += parseFloat(el.descuento);
                        ...
                        subtotal += parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100;

                        if (el.total_ice) {subtotalice += parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100;}
                        if (el.total_ice) {valorice += el.total_ice * el.cantidad;}

                        if (el.iva == 2) {subtotal12 += (parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100) + (el.total_ice*el.cantidad);}
                        if (el.iva == 1) {subtotal0 += (parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100) + (el.total_ice*el.cantidad);}
                        if (el.iva == 3) {no_impuesto += (parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100) + (el.total_ice*el.cantidad);}
                        if (el.iva == 4) {exento += (parseFloat(el.precio).toFixed(2) * el.cantidad - (el.cantidad * parseFloat(el.precio).toFixed(2) * el.descuento)/100) + (el.total_ice*el.cantidad);}
                        if(isNaN((parseFloat(parseFloat(el.precio).toFixed(2)) * parseFloat(el.cantidad) * parseFloat(el.descuento))/100)){
                            descuento += 0;
                        }else{
                            descuento += (parseFloat(parseFloat(el.precio).toFixed(2)) * parseFloat(el.cantidad) * parseFloat(el.descuento))/100;
                        }
                    }
                    valor12 = subtotal12 * 0.12;
                });
    ...
    methods: {
        ...
        recuperar(){
            axios.get('/api/factura/recuperar/' + this.$route.params.id).then( ({data}) => {
        ...
        seleccionar_cliente(tr){
            ...
            if(tr.id_plan_seguro!==null && tr.id_plan_seguro>0){
                this.factura.observacion="";
                this.factura.observacion="Seguro:"+tr.nombre_seguro+"        Plan:"+tr.nombre_plan_seguro;
            }
        ...
        verificar_claves_acceso(){
            return new Promise((resolve,reject)=>{
                ...
                axios.post('/api/nota_venta/guardar_nota_venta_clave', {factura:this.factura, guia: this.guia,transportista:this.transportista}).then(({data}) => {
                    ...
                    reject(this.factura.clave_acceso);
                });
            });
        ...
        enviarfactura(){
            axios.post('/api/factura_acumulada/guardar_notaventa_proforma', {
                id_proforma: this.$route.params.id,
                ...
                if(data=='existe clave factura'){
                    this.$vs.notify({
                        color: "danger",
                        text: "Ya existe la clave de esta nota venta"
                    });
                    return
                }
                this.id_nota_venta_recupera=data.nota_venta[0].id_nota_venta;
                console.log(data.nota_venta[0].id_nota_venta);
                 axios.get('/api/creacion_nota_venta_pdf/'+data.nota_venta[0].id_nota_venta+'/v').then( resp => {
                    // this.$vs.notify({
                    ...
                    // });
                    // this.enviado();
                    this.enviar_correo(data.nota_venta[0].id_nota_venta);
                }).catch(err=>{
                    this.$vs.notify({
                        color: "danger",
                ...
                //     color: "warning"
                // });
                   var recupera_factura = data.nota_venta[0];
        ...
        enviar_correo(id){
            axios.post('/api/nota_venta/enviarcorreo', {
                ...
                id_empresa:this.usuario.id_empresa
                    ...
                    this.$vs.notify({
                        title: "Registrado con Exito",
                        text: "Se ha registrado la Nota Venta con exito",
                        color: "success"
                    });
                    this.enviado();
            })
        ...
        //Facturación
        enviado(){
            if(this.usuario.id_empresa==65){
                this.$router.push("/facturacion/factura_acumulada");
                axios.get("/api/imprimir/ticket/nota_venta/"+this.id_nota_venta_recupera+"/d").then(resp=>{
                    setTimeout(() => {
                        window.open("/api/imprimir/ticket/nota_venta/"+this.id_nota_venta_recupera+"/d", '_top');
                    }, 300);
        ...
        //producto anterior
        precio_producto(id){
            axios.get("/api/listarproductos/nota_venta/anterior",
                {
                    params:{
                    ...
                    if(resp.data=='vacio'){
                        this.$vs.notify({
                            ...
                            text: "No existe una Nota Venta anterior de ese Cliente",
                            color: "danger"
                        });
                        return;
                    }
                    ...
                    if(resp.data=='no hay producto'){
                        this.$vs.notify({
                            ...
                            text: "No existe producto anterior en otra Nota Venta",
                            color: "danger"
                        });
                        return;
                    }
