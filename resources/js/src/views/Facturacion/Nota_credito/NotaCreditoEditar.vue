        ...
        <vx-card>
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-1/6 w-full mb-6">
                    <h6>Fecha:</h6>
                    <vs-input class="w-full" disabled :value="factura.fecha"/>
                </div>
                <div class="vx-col sm:w-1/3 w-full mb-6">
                    <h6 class="mb-1">Número Documento</h6>
                    <vs-input class="w-full" placeholder="000-000-000000000" disabled :value="factura.documento"/>
                </div>
                <div class="vx-col sm:w-1/6 w-full mb-6">
                    <h6 class="mb-1">Fecha Doc:</h6>
                    <vs-input class="w-full" disabled :value="factura.fecha_doc"/>
                </div>
                <div class="vx-col sm:w-1/3 w-full mb-6">
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-full w-full mb-6 relative" v-else>
                    ...
                    <div class="busqueda_lista busqueda_cliente_ls" v-if="cliente.busqueda.length>2">
                        ...
                        <template slot-scope="{ data }">
                            <vs-tr v-for="(tr, index) in data" :key="index" class="fila_lista">
                                ...
                                <vs-td class="estilodes">
                                    ...
                                    <span v-if="tr.descuento_comp">{{ tr.descuento_comp/tr.cantidad_dsc*tr.cantidad | currency }}</span><span v-else>0.00</span> 
                                    <template v-if="tr.p_descuento==0">%</template>
                                </vs-td>
                                <vs-td style="width:130px!important;" v-if="tr.descuento_comp">
...
export default {
    ...
    computed: {
        ...
        formulas(){
            ...
            this.producto.lista_productos.forEach(el => {
                if (el.p_descuento == 1) {
                    console.log("Descuento_comp:"+el.descuento_comp);
                    if(el.descuento_comp>0 && el.descuento_comp!==null){
                         console.log("Descuento_comp 3:"+el.descuento_comp);
                        subtotal += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);
                    }else{
                        console.log("Descuento_comp 4:"+el.descuento_comp);
                        subtotal += el.precio * el.cantidad;
                    }
                    //subtotal += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);
                    // if (el.iva == 2) {subtotal12 += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                    // if (el.iva == 1) {subtotal0 += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                    // if (el.iva == 3) {no_impuesto += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                    // if (el.iva == 4) {exento += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                    if(isNaN(parseFloat(el.descuento_comp))){
                        ...
                        if (el.iva == 3) {no_impuesto += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                        if (el.iva == 4) {exento += el.precio * el.cantidad - (el.descuento_comp/el.cantidad_dsc*el.cantidad);}
                        descuento += parseFloat(el.descuento_comp/el.cantidad_dsc*el.cantidad);
                    }
                } else {
                    console.log("Descuento_comp 2:"+el.descuento_comp);
                    if(el.descuento_comp>0 && el.descuento_comp!==null){
                        subtotal += el.precio * el.cantidad - ((el.cantidad * el.precio * el.descuento_comp)/100/el.cantidad_dsc*el.cantidad);
                    }else{
                        subtotal += Number(el.precio) * Number(el.cantidad);
                    }
                    
                    
                    if(isNaN((parseFloat(el.precio) * parseFloat(el.cantidad) * parseFloat(el.descuento_comp))/100)){
                        if (el.iva == 2) {subtotal12 += el.precio * el.cantidad ;}
                        if (el.iva == 1) {subtotal0 += el.precio * el.cantidad ;}
                        if (el.iva == 3) {no_impuesto += el.precio * el.cantidad ;}
                        if (el.iva == 4) {exento += el.precio * el.cantidad ;}
                        ...
                        if (el.iva == 2) {subtotal12 += el.precio * el.cantidad - ((el.cantidad * el.precio * el.descuento_comp)/100/el.cantidad_dsc*el.cantidad);}
                        if (el.iva == 1) {subtotal0 += el.precio * el.cantidad - ((el.cantidad * el.precio * el.descuento_comp)/100/el.cantidad_dsc*el.cantidad);}
                        if (el.iva == 3) {no_impuesto += el.precio * el.cantidad - ((el.cantidad * el.precio * el.descuento_comp)/100/el.cantidad_dsc*el.cantidad);}
                        if (el.iva == 4) {exento += el.precio * el.cantidad - ((el.cantidad * el.precio * el.descuento_comp)/100/el.cantidad_dsc*el.cantidad);}
                        descuento += parseFloat((parseFloat(el.precio) * parseFloat(el.cantidad) * parseFloat(el.descuento_comp))/100/el.cantidad_dsc*el.cantidad);
                    }
                }
            });
    ...
    methods: {
        ...
        //valores de editar
        recuperar(){
            axios.get('/api/notacredito/recuperar/' + this.$route.params.id).then( ({data}) => {
                this.factura = {
        ...
        guardar_factura(){
            ...
            axios.put('/api/notacredito/editar_factura', {
        ...
        async crearfacturacion(firma, password, factura, tipo, usuario, id_factura, carpeta){
            try { 
                ...
                let {data:quefirma} = await script_comprobantes.verificar_firma.getAll({ mensaje:certificado, tipo:tipo, id_factura:id_factura, carpeta:carpeta });
                ...
                let {data:recibida} = await script_comprobantes.autorizar_comprobante.getAll({ comprobante:comprobante, validado:validado, usuario:usuario, tipo:tipo, id_factura:id_factura, carpeta:carpeta });
