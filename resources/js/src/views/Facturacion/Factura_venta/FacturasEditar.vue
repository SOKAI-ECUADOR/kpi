        ...
        <vx-card>
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-1/3 w-full mb-6">
                    ...
                    <flat-pickr :config="configdateTimePicker" class="w-full" :disabled="disabled_modo" v-model="factura.fecha_emision" placeholder="Seleccionar"></flat-pickr>
            ...
            <vs-divider position="left" class="flexy" v-if="usuario.id_empresa==34 || usuario.id_empresa==74">
                <h3>Cuotas Extras</h3>
            ...
            <transition name="slide-fade" v-if="usuario.id_empresa==34 || usuario.id_empresa==74">
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="success" type="filled" :disabled="disabled_button" @click="verificarclaves()">GUARDAR</vs-button>
...
export default {
    ...
    computed: {
        ...
        total_pagado(){
            ...
            var retencion1=Number(retencion);
            var creditos2=Number(creditos);
            var pagas3=Number(pagas);
            if(this.interes_monto>0){
            ...
            console.log(total+":Pagado"+parseFloat(retencion1)+":retencion"+parseFloat(creditos2)+":creditos"+parseFloat(pagas3)+":pagos");
    ...
    methods: {
        ...
        verificar_claves_acceso(){
            return new Promise((resolve,reject)=>{
                if(this.modofactura == 0){
                    axios.post('/api/factura_venta/guardar_factura_clave', {
        ...
        guardar_factura(){
            ...
            if(this.modofactura == 0){
                //axios.post('/api/factura_venta/guardar_factura_clave', {factura:this.factura, guia: this.guia,transportista:this.transportista,id_empresa:this.usuario.id_empresa}).then(({data}) => {
                    axios.put('/api/factura_venta/editar_factura', {
                        ...
                        if(data=="existe clave factura"){
                            this.$vs.notify({
                                time: 8000,
                                title: "Error Clave Acceso Factura",
                                text: "Ya existe esta clave de acceso en factura",
                                color: "danger"
                            });
                            this.disabled_button=false;
                            //this.enviado();
        ...
        recuperar(){
            axios.get('/api/factura/recuperar/' + this.$route.params.id).then( ({data}) => {
                console.log("Proforma:"+data.factura.modo);
                if(data.factura.modo == 1){
                    this.modofactura = 1;
                    this.disabled_modo=true;
                    this.nombre_empresa_rec=data.factura.nombre_empresa;
                    ...
                    console.log("IDEmpresa:"+this.usuario.id_empresa);
                    this.factura = {
        ...
        verificaproducto(rec){
            axios.post('/api/factura_venta/verificaproducto', {
                ...
                data.forEach(el => {
                    var subtotal = 0;
                    var descuento = 0;
                    var exist_ce1=this.nombre_empresa_rec.indexOf("C.E. FUEGOS");
                    var exist_ce2=this.nombre_empresa_rec.indexOf("C.E.FUEGOS");
                    if(exist_ce1!==-1 || exist_ce2!==-1){
                        if(el.sector==1 && this.empresa.negativo==0 &&  (el.id_producto_bodega==="undefined" || el.id_producto_bodega == null)){
                            el.cantidad = 0;
        ...
        decimales_producto(index){
            this.producto.lista_productos[index].precio=parseFloat(this.producto.lista_productos[index].precio).toFixed(2);
        },
        //correos
