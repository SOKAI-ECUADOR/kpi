    ...
    <div id="invoice-page">
        <vx-card>
            <vs-divider position="center">
                <h3 v-if="lista.factura.clave_acceso">Factura N° {{(lista.factura.clave_acceso).substring(24,27)}}-{{(lista.factura.clave_acceso).substring(27,30)}}-{{(lista.factura.clave_acceso).substring(30,39)}}</h3>
                    ...
                    <vs-table hoverFlat :data="lista.productos" style="font-size: 12px;">
                        ...
                        <template slot-scope="{ data }">
                            <vs-tr v-for="(tr, index) in data" :key="index" class="fila_lista">
                                ...
                                <vs-td v-if="tr.descuento"> $ {{ tr.descuento}}</vs-td>
                                <vs-td v-else> $ 0.00</vs-td>
            ...
            <vs-divider position="left" class="flexy" v-if="lista.iva.length || lista.renta.length">
                <h3>Retenciones</h3>
            </vs-divider>
            <transition name="slide-fade">
                <div class="vx-row leading-loose p-base">
                    <div class="w-full">
                            ...
                            <div class="w-2/3 ml-auto mr-auto">
                                <div class="vx-row">
                                    <div class="vx-col md:w-3/5 w-full mb-2 ml-auto text-center">
                                        ...
                                        <div class="mt-2" v-if="lista.iva[index].descrip_retencion">$ {{lista.iva[index].descrip_retencion}}</div>
                                        <div class="mt-2" v-else>-</div>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <label class="vs-input--label">Base</label>
                                        <div class="mt-2">$ {{lista.iva[index].baseiva}}</div>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                        ...
                        <div class="vx-row hovertrash" v-for="(tr, index) in lista.renta" :key="index">
                             <div class="w-2/3 ml-auto mr-auto">
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 w-full mb-2 mr-auto text-center">
                                        <label class="vs-input--label">Valores por RENTA</label>
                                        <div class="mt-2" v-if="lista.renta[index] && lista.renta[index].descrip_retencion">$ {{lista.renta[index].descrip_retencion}}</div>
                                        <div class="mt-2" v-else>-</div>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <label class="vs-input--label">Base</label>
                                        <div class="mt-2" v-if="lista.renta[index] && lista.renta[index].baserenta">$ {{lista.renta[index].baserenta}}</div>
                                        <div class="mt-2" v-else>-</div>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <label class="vs-input--label">Porcentaje</label>
                                       <div class="mt-2" v-if="lista.renta[index] && lista.renta[index].porcentajerenta">{{lista.renta[index].porcentajerenta}}</div>
                                       <div class="mt-2" v-else>-</div>
                                    </div>
                                    <div class="flex-1 mb-2 text-center">
                                        <label class="vs-input--label">Val. Ret.</label>
                                        <div class="mt-2" v-if="lista.renta[index] && lista.renta[index].cantidadrenta">$ {{lista.renta[index].cantidadrenta}}</div>
                                        <div class="mt-2" v-else>-</div>
                                    </div>
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="danger" type="filled" to="/facturacion/factura-venta">ATRAS</vs-button>
            </div>
        </vx-card>
    </div>
</template>
...
export default {
    ...
    methods: {
        listar(){
            axios.get('/api/facturaver/' + this.$route.params.id).then( ({data}) => {
                ...
                let ice = 0;
                console.log(this.lista.iva[0].descrip_retencion);
