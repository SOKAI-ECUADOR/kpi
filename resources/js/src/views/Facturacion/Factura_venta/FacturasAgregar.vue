    ...
    <div id="invoice-page">
        <vx-card>
            <vs-divider position="center">
                <h3 v-if="factura.clave_acceso">Factura N° {{(factura.clave_acceso).substring(24,27)}}-{{(factura.clave_acceso).substring(27,30)}}-{{(factura.clave_acceso).substring(30,39)}}</h3>
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-1/3 w-full mb-6">
                    ...
                    <flat-pickr :config="configdateTimePicker" class="w-full" v-model="factura.fecha_emision" placeholder="Seleccionar" @on-change="listarclave()"></flat-pickr>
                ...
                <div class="vx-col sm:w-1/4 w-full mb-6" style="margin-top: 20px; margin-bottom: 0.2rem !important;">
                ...
                <div class="vx-col sm:w-1/4 w-full mb-6" style="margin-top: 20px; margin-bottom: 0.2rem !important;">
                    <vs-checkbox icon-pack="feather" icon="icon-check" v-model="existe_iva_8">
                        <template v-if="existe_iva_8">
                            <label class="vs-input--label" style="font-size: 14px;font-weight: bold;">Si</label>
                        </template>
                        <template v-else>
                            <label class="vs-input--label" style="font-size: 14px;font-weight: bold;">No</label>
            ...
            <div class="vx-row leading-loose p-base" v-if="guia">
                ...
                <div class="vx-col sm:w-1/3 w-full mb-3 ml-auto" style="text-align: center;">
                    ...
                    <flat-pickr :config="configdateTimePicker" class="w-full mt-1" v-model="transportista.fecha_inicio_transporte" placeholder="Seleccionar" @on-change="listarclave_guia()"></flat-pickr>
                        ...
                        <template slot-scope="{ data }">
                            ...
                            <vs-td v-if="tr.cod_alterno">{{tr.cod_alterno}}</vs-td><vs-td v-else>{{ tr.cod_principal }}</vs-td>
                                ...
                                <vs-td style="width:70px!important;">
                                    ...
                                    <!-- <vs-td style="width:70px!important;" v-if="typeof tr.precio_sin_iva=='undefined' || tr.precio_sin_iva==''">
                                    ...
                                    <vs-td style="width:70px!important;" v-if="usuario.id_empresa==59">
                                ...
                                <feather-icon icon="DollarSignIcon" svgClasses="w-6 h-6 hover:text-primary stroke-current cursor-pointer" class="eliminar_producto_icono" @click="precio_producto(index)" v-if="usuario.id_empresa==59"/>
                    ...
                    <div class="busqueda_lista busqueda_producto_ls" style="display: none;">
                        <div v-if="preloader.productos">
                            <ul class="ul_busqueda_lista">
                                <li v-for="(tr,index) in producto.productos" :key="index" @click="seleccionar_productos(tr)"> <span v-if="tr.cod_alterno" style="font-weight: bold;" >CodAlt: {{ tr.cod_alterno }} - </span> <span v-else style="font-weight: bold;">CódPrin: {{tr.cod_principal}} - </span><span style="font-weight: bold;">{{ tr.nombre }}</span> <span v-if="tr.presentacion" style="font-weight: bold;" > - Presentación: {{ tr.presentacion }} </span> <span v-if="tr.nombrebodega"> - <span style="font-size: 12px;">Código: {{tr.cod_principal}}</span>- <span style="font-size: 12px;">Bodega: {{tr.nombrebodega}}</span> - <span style="font-size: 12px;">cantidad: {{tr.cantidad}}</span></span> <span v-if="tr.nombre_marca && usuario.id_empresa==68"><span style="font-size: 12px;">Marca: {{tr.nombre_marca}}</span></span> </li>
                            </ul>
                        </div>
                        <div v-else>
                            <ul class="ul_busqueda_lista lista_preloader">
                ...
                <div class="vx-col w-full">
                    <div class="vx-row" v-if="producto.tipo">
                        <div class="vx-col sm:w-1/2 w-full">
                            <h6>Observaciones:</h6>
                            <vs-textarea  class="w-full"  v-model="factura.observacion"  rows="5" maxlength="300"/>
                        </div>
                        <div class="vx-col sm:w-1/2 w-full">
                            <div class="cabezera_total">
                                ...
                                <div v-if="formulas.subtotal12>0">SUBTOTAL IVA {{valor_iva}}% <span>$ {{ formulas.subtotal12 }}</span></div>
                                <div v-if="formulas.valor12>0">Valor IVA {{valor_iva}}% <span>$ {{ formulas.valor12 }}</span></div>
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-1/3 w-full mb-2 text-center">
                    <label class="vs-input--label">SALDO PAGADO</label>
                    <h1>$ {{ total_pagado }}</h1>
                </div>
            </div>
                            ...
                            <div class="w-2/3 ml-auto mr-auto">
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 w-full mb-2 ml-auto text-center">
                                        ...
                                        <vs-select @change="agregarretencioniva(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.iva">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" class="w-full derecha" @keyup="agregarretencionivavalor(index, tr.baseiva)" v-model="tr.baseiva"/>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                ...
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 w-full mb-2 mr-auto text-center">
                                        ...
                                        <vs-select @change="agregarretencionrenta(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.renta">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" placeholder="0.00" class="w-full derecha" @keyup="agregarretencionrentavalor(index, tr.baserenta)" v-model="tr.baserenta"/>
                                        <div v-show="error.error" v-if="parseFloat(tr.baserenta)<=0 || !tr.baserenta">
                                            <div v-for="err in tr.errorbase" :key="err" v-text="err" class="text-danger"></div>
                                        </div>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                    ...
                    <div class="w-full">
                        <div class="vx-row hovertrash" v-for="(tr,index) in pagos.datos" :key="index">
                            ...
                            <div class="vx-col sm:w-1/6 w-full mb-2 text-center mr-auto" v-if="tr.metodo_pago=='Anticipo'">
                                <vs-input class="w-full text-center" label="Anticipo Total" disabled :value="anticipoexistente"/>
                            </div>
                            <div class="vx-col sm:w-1/6 w-full mb-2 text-center" v-if="tr.metodo_pago!='Anticipo'">
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="success" type="filled" :disabled="disabled" @click="guardar_factura()">GUARDAR</vs-button>
            ...
            <vs-popup :title="modal.titulo" :active.sync="modal.abrir" class="modal-xl">
                ...
                <!-- <div class="con-exemple-prompt">
                    <div class="vx-row">
                        <div
                            class="vx-col  sm:w-1/5 w-full relative mb-6"
                            v-if="codigocliente"
                        >
                            <vs-input
                                class="w-full"
                            ...
                            <div v-show="error" v-if="!codigocliente">
                                <div
                                    v-for="err in errorcodigocliente"
                                    :key="err"
                                    v-text="err"
                                    class="text-danger"
                                ></div>
                        ...
                        >
                            <vs-input
                                ...
                                @input="nombreaddicional(crear_cliente.nombre)"
                            />
                            <div v-show="error" v-if="!crear_cliente.nombre">
                                <div
                                    v-for="err in error_cliente.crear_cliente
                                        .nombre"
                                    :key="err"
                                    v-text="err"
                                    class="text-danger"
                        ...
                        >
                            ...
                            <vs-select
                                ...
                                @change="
                                    cambio(crear_cliente.tipo_identificacion)
                                "
                            >
                                <vs-select-item
                                    :key="index"
                            ...
                            <div
                                v-show="error"
                                v-if="!crear_cliente.tipo_identificacion"
                            >
                                <div
                                    v-for="err in error_cliente.crear_cliente
                                        .tipo_identificacion"
                        ...
                        >
                            ...
                            <div
                                v-show="error"
                                v-if="
                                    crear_cliente.identificacion.length != 13 &&
                                        crear_cliente.identificacion.length !=
                                            10
                                "
                            >
                                <div
                        ...
                        >
                            ...
                            <div
                                v-show="error"
                                v-if="!crear_cliente.grupo_tributario"
                            >
                                <div
                                    v-for="err in error_cliente.crear_cliente
                                        .grupo_tributario"
                        ...
                        <div
                            class="vx-col sm:w-1/4 sm:w-1/2 w-full relative mb-6"
                        >
                            <label class="vs-input--label">E-mail</label>
            ...
            <vs-popup title="Escoja la impresora" :active.sync="modal_impresora">
                <vs-select
                            ...
                            <br>
                <div>
                    <vs-button color="success" type="filled"  @click="imprimir(nombre_impresora)">Imprimir</vs-button> 
                    <vs-button color="danger" type="filled" @click="cancelar_impresion()">Cancelar</vs-button> 
                </div>
            </vs-popup>
        </vx-card>
    </div>
...
export default {
        ...
        formulas(){
            ...
            if(this.usuario.id_empresa==59){
                ...
                this.producto.lista_productos.forEach(el => {
                    ...
                    if(this.existe_iva_8){
                        valor12 = subtotal12 * 0.08;
                        this.valor_iva=8;
                    }else{
                        valor12 = subtotal12 * 0.12;
                        this.valor_iva=12;
    ...
    methods: {
        ...
        listarcanton(id){
            if(id==null){
                axios.get('/api/notacredito/listar_canton/17').then( ({data}) => {
        ...
        cambio(c) {
            ...
            if (c === "Consumidor Final") {
                this.crear_cliente.identificacion = "9999999999999";
            }
        },
        ...
        guardar_factura(){
            this.disabled=true;
            //valida registros
            if(this.validar()){this.disabled=false;return;}
            ...
            //verifica si hay un pendiente de apgo de la factura todavía existente
            if(parseFloat(n1-n2).toFixed(2)>0 || parseFloat(n1-n2).toFixed(2)<0){
                
                this.$vs.notify({
                    time: 8000,
                    title: "No se puede Guardar la factura",
                    text: "Todavía existe un saldo pendiente de $ "+parseFloat(n1-n2).toFixed(2),
                    color: "danger"
                });
                this.disabled=false;
                return;
            }
             //verifica si hay un pendiente de apgo de la factura todavía existente por segunda vez
            ...
            var alerta = false;
            var valoranticipo = 0;
            //recorre los valores del anticipo y hace una suma general
            this.pagos.datos.forEach(el => {
            ...
            //verfica si el anticipo existente del cliente es mayor o igual al anticipo hecho en la factura
            //si es caso contrario manda una alerta del valor excedido
            if(alerta){
                if(parseFloat(valoranticipo)>parseFloat(this.anticipoexistente)){
                    this.$vs.notify({
                        time: 8000,
            ...
            //verifica el numero de la clave de acceso si esta repetida, de ser asi obtiene el numero de la secuencial del punto de emisión
            axios.post('/api/factura_venta/guardar_factura_clave', {factura:this.factura, guia: this.guia,transportista:this.transportista,id_empresa:this.usuario.id_empresa}).then(({data}) => {
                if(data.guia == "repetido"){
                    ...
                    //verifica si es repetido el numero de clave de acceso al ser asi genera una nueva clave de acceso y genera su módulo 11
                if(data.factura == "repetido"){
                        var url = "/api/listarclave/" + this.usuario.id;
                        axios.get(url).then(res => {
                            //recupera la fecha de emision de la factura con el formato de DDMMYYYY compromete los 8 primeros degitos de la clave
                            var fecha = moment(this.factura.fecha_emision).format("DDMMYYYY");
                            //recupera datos generales de la empresa
                            var rec = res.data.recupera[0];
                            //recupera la secuencial de la factura que se obtiene del punto de emisión que consta de 9 dígitos
                            var secuencial = this.zeroFill(res.data.secuencial, 9);
                            //recupera el establecimiento que se emite esta factura que consta 3 digitos
                            var establecimiento = this.zeroFill(rec.establecimiento, 3);
                            //recupera el punto de emision que se emite esta factura que consta 3 digitos
                            var punto_emision = this.zeroFill(rec.punto_emision, 3);
                            //se genera la clave de acceso de 48 digitos sin modulo conformado como:
                            //fecha 8 dígitos
                            //01 (tipo de factura, revisar documentacion del sri para la numeración)
                            //ruc de la empresa
                            //ambiente del envio (revisar documentacion del sri para la numeración)
                            //numero de establecimiento 3 dígitos
                            //punto de emisión 3 dígitos
                            //secuencial 8 dígitos
                            //12345678 numeración aleatria de 8 dígitos impuesto por el SRI (puede ser editada a gusto personal)
                            //tipo de emision siendo 1 el emision normal
                            var codigoacc = fecha+"01"+rec.ruc_empresa+rec.ambiente+establecimiento+punto_emision+secuencial+"12345678"+1;
                            //se envia dicho registro al modulo 11 para general el modulo 11
                            var acceso = this.Modulo11(codigoacc);
                            this.factura.clave_acceso = codigoacc + acceso;
                            this.enviarfactura();
                        }).catch( err => {
        ...
        enviarfactura(){
            axios.post('/api/factura_venta/guardar_factura', {
                ...
                pagos:this.pagos,
                //si la factura tiene iva 8%
                existe_iva_8:this.existe_iva_8,
                // transportista
                guia: this.guia,
                transportista: this.transportista,
                //productos factura anterior
                anterior_producto:this.anterior_producto,
                // tabla interes
                ...
                if(data=="factura clave"){
                    this.$vs.notify({
                        time: 8000,
                        title: "Error al guardar Factura",
                        text: "La clave acceso de la factura ya existe",
                        color: "danger"
                    });
                    return;
                }
                if(data=="guia clave"){
                    this.$vs.notify({
                        time: 8000,
                        title: "Error al guardar Factura",
                        text: "La clave acceso de la guia ya existe",
                        color: "warning"
                    });
                    return;
                }
                ...
                this.id_factura_imp=recupera_factura.id_factura;
                ...
                this.$vs.notify({
                    ...
                    text: 'La factura no pudo ser enviada, intente mas tarde',
                    color: "danger"
                });
            });
        },
        ...
        listarclave() {
            if (!this.$route.params.id) {
                ...
                axios.get(url).then(res => {
                    ...
                    var codigoacc = fecha+"01"+rec.ruc_empresa+rec.ambiente+establecimiento+punto_emision+secuencial+"12345678"+1;
                    var acceso = this.Modulo11(codigoacc);
                    this.exist_interes=res.data.exist_interes;
                    //al terminar el módulo 11 genera  la clave de acceso completo de 49 dígitos
                    this.factura.clave_acceso = codigoacc + acceso;
                });
                return false;
            }
        },
        //verifica que sea un numero como string para que concatene y no sume
        ...
        agregarretencionivavalor(index, valor){
            var total = parseFloat(this.formulas.valor12).toFixed(2) - parseFloat(valor).toFixed(2);
        ...
        validar(){
            ...
            for (var i = 0; i < this.producto.lista_productos.length; i++) {
                ...
                if (!this.producto.lista_productos[i].cantidad) {
                    ...
                    if (parseFloat(this.producto.lista_productos[i].cantidad)<=0) {
                        this.producto.lista_productos[i].errorcantidad.push("Obligatorio");
                        this.error.error = 1;
                    }
                }
        ...
        validar_crear_cliente(){
            ...
            if(this.crear_cliente.tipo_identificacion==1){
                ...
                if(this.crear_cliente.tipo_identificacion>2){
                    if(this.crear_cliente.identificacion.length<=0){
        ...
        //Facturación
        enviado(){
            if(this.usuario.id_empresa==60 || (this.usuario.id_empresa==50 && this.usuario.id_punto_emision==43)){
                axios.get("/api/imprimir/ticket/"+this.id_factura_imp+"/d").then(resp=>{
                    window.open("/api/imprimir/ticket/"+this.id_factura_imp+"/d", '_blank');
                    setTimeout(() => {
                        this.$router.push("/facturacion/factura-venta");
                    }, 200);

                }).catch(err=>{
                    console.log("ERROR[Imprimir ticket]"+err);
                });
            }else{
                // if(this.usuario.id_empresa==59){
        ...
        cambiarivas(id,valor){
            if(valor!==0){
                this.producto.lista_productos[id].precio=parseFloat(valor).toFixed(2)/1.12;
            }
        },
        //metodos impresora
        obtenerImpresora(){
            ...
            .then(resp=>{
                ...
                console.log("[ERROR TRAER IMPRESORA]::"+err);
            });
            // var comprobante=0;
        ...
        //productos factura anterior
        precio_producto(id){
            axios.get("/api/listarproductos/factura/anterior",
                {
                    params:{
                        id:this.cliente.id_cliente,
                        id_pto_emision:this.usuario.id_punto_emision,
                        id_producto:this.producto.lista_productos[id].id_producto,
                    }
                }).then(resp=>{
                    if(resp.data=='vacio'){
                        this.$vs.notify({
                            title: "Error Productos",
                            text: "No existe una factura anterior de ese Cliente",
                            color: "danger"
                        });
                        return;
                    }
                    if(resp.data=='no existe'){
                        this.$vs.notify({
                            title: "Error Cliente",
                            text: "No existe cliente",
                            color: "danger"
                        });
                        return;
                    }
                    if(resp.data=='no hay producto'){
                        this.$vs.notify({
                            title: "Error Producto",
                            text: "No existe producto anterior en otra Factura",
                            color: "danger"
                        });
                        return;
                    }
                    //this.producto.tipo = true;
                    console.log("prducto anterior:"+resp.data);
                    var subtotal=0
                    this.producto.lista_productos[id].precio=resp.data[0].precio;
                    
                    $(".focuspr").focus();
                }).catch(err=>{
                    console.log("[ERROR:: traer productos del anterior factura]"+err);
                });
        },
        productos_anterior(){
            axios.get("/api/listarproductos/factura/anterior",
                {
                    ...
                    this.producto.tipo = true;
                    console.log("prducto anterior:"+resp.data[0].cod_principal);
                    var subtotal=0
                    resp.data.forEach(el=>{
                        subtotal =  (el.precio - el.descuento).toFixed(2);
                        this.producto.lista_productos.push({
