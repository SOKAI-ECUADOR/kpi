...
export default {
    ...
    computed: {
        ...
        total_pagado(){
            ...
            if(!retencion || isNaN(retencion)){
                retencion = 0;
            }
            if(!creditos || isNaN(creditos)){
                creditos = 0;
            }
            if(!pagas || isNaN(pagas)){
                pagas = 0;
            }
    ...
    methods: {
        ...
        guardar_factura(){
            ...
            var totalrrenta = 0;
            this.valorretenciones.forEach(el => {
                totalriva += el.baseiva;
                totalrrenta += el.baserenta;
            });
            if(this.formulas.valor12>0){
                if(totalriva>this.formulas.valor12){
                    this.$vs.notify({
                        time: 8000,
                        title: "Error en base iva",
                        text: "El valor Base iva excede el valor del iva de la fctura",
                        color: "danger"
                    });
                    return;
                }
            }
            if(this.formulas.subtotal>0){
                if(totalrrenta>this.formulas.subtotal){
                    this.$vs.notify({
                        time: 8000,
                        title: "Error en base renta",
                        text: "El valor Base renta excede el valor del iva de la fctura",
                        color: "danger"
                    });
                    return;
                }
        ...
        listarvendedor(){
            axios.get("/api/factura/vervendedor?empresa="+this.usuario.id+"&id_user="+this.usuario.id).then(({data}) => {
        ...
        enviado() {
            if (
                this.usuario.id_empresa == 60 ||
                (this.usuario.id_empresa == 50 &&
                    this.usuario.id_punto_emision == 43)
            ) {
                this.imprimir_ticket().then(val=>{
                    console.log("SE ENVIA RUTA");
        ...
        imprimir_ticket(){
            return new Promise((resolve,reject) => {
                axios.get("/api/imprimir/ticket/" + this.id_factura_imp + "/d")
                                .then(resp => {
                                    window.open(
                                        "/api/imprimir/ticket/" +
                                            this.id_factura_imp +
                                    ...
                                    console.log("SE EJECUTO EL TICKET");
                                    resolve(this.id_factura_imp);
                                    // setTimeout(() => {
                                    //     this.$router.push("/facturacion/factura-venta");
                                    // }, 400);
                                })
                                .catch(err => {
                                    reject("ERROR[Imprimir ticket]" + err);
                                    this.$router.push("/facturacion/factura-venta");
                                });
            });
    ...
    created(){
        ...
        axios.get("/api/factura/duplicar?id=" + duplicar).then(({data}) => {
            ...
            if(data.factura.existe_iva_8>0){
                this.existe_iva_8=true;
            }else{
                this.existe_iva_8=false;
            }
            if(this.usuario.id_empresa==59){
