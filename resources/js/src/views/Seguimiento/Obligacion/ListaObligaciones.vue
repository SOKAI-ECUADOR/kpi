    ...
    <vx-card>
      ...
      <vs-table stripe :data="contenidotipo">
        ...
        <template slot-scope="{data}">
          <vs-tr :key="datos.id_obligacion" v-for="(datos, index) in data">
            <vs-td style="width: 80px;">{{index+1}}</vs-td>
            <vs-td>{{datos.descripcion_obligacion}}</vs-td>
            <vs-td class="whitespace-no-wrap">
              <feather-icon v-if="editarrol" icon="EditIcon" class="cursor-pointer" svgClasses="w-5 h-5 hover:text-primary stroke-current" @click.stop="verObligacion(datos.id_obligacion, datos.descripcion_obligacion)" />
              <feather-icon v-if="eliminarrol" icon="TrashIcon" class="cursor-pointer" svgClasses="w-5 h-5 hover:text-danger stroke-current"  @click.stop="eliminarTipo(datos.id_obligacion)"/>
            </vs-td>
            </vs-tr>
          </template>
      </vs-table>
      ...
      <vs-popup :title="titulomodal" :active.sync="activePrompt">
          <div class="vx-row">
              <div class="vx-col sm:w-full w-full">
                ...
                <vs-input class="w-full" v-model="!descripcionrecupera" rows="3" maxlength="199"/>
                <div v-show="error" v-if="!descripcionrecupera">
...
export default {
  ...
  methods: {
    ...
    vaciarTipo(){
      this.descripcionrecupera="";
    },
    //listar obligacion
    verObligacion(id, descripcion){
      if(id){
        this.idrecupera = id;
        this.descripcionrecupera = descripcion;
        this.activePrompt=true;
    ...
    acceptAlert(parameters) {
      ...
      .then(res =>{
        this.$vs.notify({
          ...
          text: "El registro selecionado fue eliminado con exito"
        });
        this.listar(1, this.buscar);
      }).catch(err => {
        this.$vs.notify({
    ...
    listar(page, buscar) {
      let me = this;
      var url =
        "/api/seguimiento/obligacion?buscar=" +
        buscar;
