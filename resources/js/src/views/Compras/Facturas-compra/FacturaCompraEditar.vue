        ...
        <vx-card>
            ...
            <div class="vx-row leading-loose p-base">
                ...
                <div class="vx-col sm:w-1/6 w-full mb-6">
                    ...
                    <flat-pickr :config="configdateTimePicker" class="w-full" v-model="factura.fecha_emision" @on-change="listarclave()" placeholder="Seleccionar"></flat-pickr>
                ...
                <div class="vx-col sm:w-1/3 w-full mb-6">
                    ...
                    <vs-select placeholder="buscar" autocomplete class="selectExample w-full" v-model="factura.tipo_sustento">
                ...
                <div class="vx-col sm:w-1/4 w-full mb-6">
                    ...
                    <vs-select placeholder="buscar" autocomplete class="selectExample w-full" v-model="factura.destino_pago">
                ...
                <div class="vx-col sm:w-1/6 w-full mb-6">
                    ...
                    <ul class="demo-alignment">
                      <li style="margin: 13px 1.5rem;">
                        <vs-radio v-model="factura.gastos" vs-value="1g">Si</vs-radio>
                      </li>
                      <li style="margin: 13px 1.5rem;">
                        <vs-radio v-model="factura.gastos" vs-value="0g">No</vs-radio>
                      </li>
                    </ul>
                </div>
                <div class="vx-col sm:w-2/5 w-full mb-6" v-if="parseInt(factura.gastos)==1">
                    <h6 class="mb-1">Nro Importacion:</h6>
                    <vs-select class="selectExample w-full" placeholder="Seleccione una importacion" v-model="factura.importacion">
                        <vs-select-item :key="index" :value="item.id_importacion" :text="item.nombre_proveedor" v-for="(item, index) in imports"/>
                    </vs-select>
                </div>
                ...
                <div class="vx-col sm:w-1/5 w-full mb-6 mr-auto">
                    ...
                    <ul class="demo-alignment">
                      <li style="margin: 13px 1.5rem;">
                        <vs-radio v-model="factura.docutributario" vs-value="1">Si</vs-radio>
                      </li>
                      <li style="margin: 13px 1.5rem;">
                        <vs-radio v-model="factura.docutributario" vs-value="0">No</vs-radio>
                      </li>
                    </ul>
                </div>
            </div>
                        ...
                        <template slot-scope="{ data }">
                            <vs-tr v-for="(tr, index) in data" :key="index" class="fila_lista">
                                ...
                                <vs-td class="nombrearreglo">
                                    ...
                                    <vx-tooltip v-else :text="tr.nombre" position="top" style="display: inline-flex">{{ tr.nombre }}</vx-tooltip>
                                </vs-td>
                                <vs-td>
                                ...
                                <vs-td style="width:100px!important;">
                                    ...
                                    <vs-input class="w-full derecha" :disabled="porOrden==1"  v-model="tr.cantidad"/>
                                ...
                                <vs-td style="width:100px!important;">
                                    ...
                                    <vs-input class="w-full derecha" :disabled="porOrden==1" v-model="tr.precio"/>
                                ...
                                <vs-td style="width:175px!important;" v-else>
                                    <vs-select placeholder="buscar" :disabled="exist_orden_compra" autocomplete class="selectExample w-full" v-model="tr.id_bodega">
            ...
            <vs-divider position="left" class="flexy">
                <h3>Retenciones</h3>
                <vs-switch vs-icon-on="check" :disabled="factura.respuesta == 'Enviado' || factura.cuentas>=1" color="success" class="ml-2" v-model="retenciones.estado" @click="cambiosretenciones()" vs-value="Si" style="margin-top: 4px;">
                    <span slot="off">No</span>
                </vs-switch>
            </vs-divider>
            <transition name="slide-fade">
                <div class="vx-row leading-loose p-base" v-show="retenciones.estado">
                    <div class="w-full">
                        ...
                        <p style="text-align: center;font-weight: bold;font-size: 18px;margin-bottom: 8px;" v-else>Retención N° </p>
                        <div class="vx-row hovertrash" v-for="(tr, index) in valorretenciones" :key="index">
                            <div class="w-2/3 ml-auto mr-auto">
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 text-center">
                                        ...
                                        <vs-select :disabled="factura.respuesta == 'Enviado' || factura.cuentas>=1" @change="recuperacion=true, agregarretencioniva(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.iva">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" class="w-full derecha" :disabled="factura.respuesta == 'Enviado' || factura.cuentas>=1" v-model="tr.baseiva" @click="recuperacion=true" @keyup="agregarretencionivavalor(index, tr.baseiva)"/>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                ...
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 w-full mb-2 mr-auto text-center">
                                        ...
                                        <vs-select :disabled="factura.respuesta == 'Enviado' || factura.cuentas>=1" @change="recuperacion=true, agregarretencionrenta(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.renta">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" class="w-full derecha" :disabled="factura.respuesta == 'Enviado' || factura.cuentas>=1" v-model="tr.baserenta" @click="recuperacion=true" @keyup="agregarretencionrentavalor(index, tr.baserenta)"/>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                            ...
                            <feather-icon v-if="factura.respuesta != 'Enviado' && factura.cuentas<1" icon="TrashIcon" style="position: absolute !important;right: 125px;margin-top: 80px;display: none;" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2 cursor-pointer trasher" @click="eliminararrayretencion(index)"/>
                            <feather-icon v-if="factura.respuesta != 'Enviado' && factura.cuentas<1" @click="addretenciones()" icon="PlusIcon" style="position: absolute !important;right: 125px;margin-top: 55px;display: none;" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2 cursor-pointer trasher"/>
                        ...
                        <div class="vx-row hovertrash" v-for="(tr,index) in pagos.datos" :key="index">
                            ...
                            <div class="vx-col sm:w-1/6 w-full mb-2" v-if="tr.metodo_pago!='Anticipo'">
                                ...
                                <vx-input-group>
                                    ...
                                    <template slot="append">
                                        <div class="append-text btn-addon">
                                        <vs-button color="primary" @click="abrir_plan_cuentas_pagos1(index)">Buscar</vs-button>
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="success" type="filled" :disabled="disbled_editar"  @click="pago_efectuado()">GUARDAR</vs-button>
...
export default {
    ...
    methods: {
        ...
        guardar_factura(){
            console.log("Entro al Guardar 1");
            console.log(this.porOrden);
            this.disbled_editar=true;
            if(this.validar()){this.disbled_editar=false;console.log("Entro al Guardar 1.0.0  existe un saldo pendiente");return;}
            ...
            if(this.total_pendiente>0){
                ...
                console.log("Entro al Guardar 1.0.1  existe un saldo pendiente");
                return;
            }
            if(this.total_pendiente<0){
                this.$vs.notify({
                ...
                this.disbled_editar=false;
                console.log("Entro al Guardar 1.0.2 NO guardar un saldo en negativo");
                return;
            }
            ...
            if(this.pago_hecho=='si'){
                    ...
                    this.disbled_editar=false;
                    console.log("Entro al Guardar 1.0.3 Ya se ha hecho un pago correspondiente a esta factura");
                    return;
            }
            var alerta = false;
            var valoranticipo = 0;
            ...
            if(alerta){
                if(parseFloat(this.anticipoexistente) + parseFloat(this.anticipo_creado) - parseFloat(valoranticipo) < 0){
                    ...
                    this.disbled_editar=false;
                    console.log("Entro al Guardar 1.0.4 valor a pagar excede el anticipo maximo");
                    return;
                }
            }
        ...
        validar_clave_retencion(){
            console.log("Entro al Guardar 1.1 retencion estado");
                    ...
                    if (res.data == "repetido") {
                        console.log(res.data+"entro validacion ");
                                ...
                                axios.get(url).then(res => {
                                    ...
                                    console.log("Entro al Guardar 1.1.1 retencion clave");
                                    ...
                                    console.log("Entro al Guardar 1.1.2 retencion clave");
                                    this.disbled_editar=false;
                                    reject(err);
                                    return;
                        ...
                        console.log("Entro al Guardar 1.1.3 retencion clave");
                        resolve(this.factura.clave_acceso);
                    }
                }).catch(err=>{
                    console.log("Entro al Guardar 1.1.4 retencion clave");
                    console.log(err);
                    reject(err);
                });
            });
        ...
        editar_factura(){
            ...
            //                 this.sololotes.push({
            //                     index:conteo,
            //                     id_producto_bodega_lotes:dl.id_producto_bodega_lotes,
            ...
            console.log("Entro al Guardar 2");
                axios.put('/api/factura_compra/editar_factura', {
                    ...
                    // solo si era una orden compra
                    exist_orden_compra:this.exist_orden_compra,
                    //productos lotes
                    lotes_producto:this.sololotes,
                    pro: [],
                    por_orden: this.porOrden
                }).then( ({data}) => {
                    ...
                    if(data == "error ctas_pagar"){
                            this.$vs.notify({
                                time: 8000,
                                title: "Error de registro",
                                text:
                                    "Error no coincide con el credito guardado",
                                color: "danger"
                            });
                            this.disbled_editar=false;
                            return;
        ...
        pago_efectuado(){
            ...
            axios.get("/api/factura_compra/ctaxpagar/verficacion",{
                ...
                console.log("Entro al Guardar 0");
        ...
        listarimportaciones() {
          axios.get("/api/traerimport_editar?empresa=" + this.usuario.id_empresa).then( response => {
        ...
        recuperardatos(){
            axios.get("/api/factura_compra/recuperar/"+ this.$route.params.id).then( ({data}) => {
                console.log(data);
                if(data.factura.estado_orden!==null){
                    this.exist_orden_compra=false;
                }
                
                console.log("Orden Compra");
                console.log(this.exist_orden_compra);
                ...
                console.log("fact", data.factura.descripcion);
                if(data.factura.descripcion==null){
                    this.porOrden = 0;
                }
                else{
                    this.porOrden = 1;
                ...
                this.producto.tipo = true;
                this.pp_descuento=data.factura.pp_descuento;
                this.propinapr=data.factura.propinapr;
                ...
                data.detalle_factura.forEach(el => {
                    this.producto.lista_productos.push({
                            ...
                            iva: el.id_iva,
                            ice: el.id_ice,
                            id_bodega:el.id_bodega,
                            nombrebodega: el.nombrebodega,
                            lotes:[],
                            importacion:el.importacion=='1' || el.importacion==1 ? true:false
                    });
                });
                var lotes_producto=[];
                this.producto.lista_productos.forEach(el=>{
                    if(data.productos_lote.length>0){
                        data.productos_lote.forEach(dl=>{
                            if(el.id_detalle_factura_compra==dl.id_detalle_factura_compra){
                                el.lotes.push({
                                    id_producto_bodega_lotes:dl.id_producto_bodega_lotes,
                                    cod_principal: el.cod_principal,
                                    nombre: el.nombre,
                                    cantidad: dl.cantidad_original,
                                    lote: dl.nombre,
                                    fecha_fabricacion: dl.fecha_fabricacion,
                                    fecha_vencimiento: dl.fecha_vencimiento,
                                    id_producto: el.id_producto,
                                    id_proyecto: el.proyecto,
                                    id_bodega: el.id_bodega
                                });
                            }
                        });
        ...
        validar(){
            ...
            if(!this.factura.nfactura){
                this.error.factura.numero_factura.push("Debe ingresar numero factura");
                this.error.error=1;
                console.log(2.0);
            }else{
            ...
            for (var i = 0; i < this.producto.lista_productos.length; i++) {
                ...
                if(this.producto.lista_productos[i].sector==1){
                    if(!this.producto.lista_productos[i].id_bodega){
                        this.producto.lista_productos[i].errorid_bodega.push("Obligatorio");
                        this.error.error = 1;
                        console.log(7.2);
                    }
                }
