<template>
    <div id="invoice-page">
        <vx-card>
                    ...
                    <vs-table hoverFlat :data="producto.lista_productos" style="font-size: 12px;">
                        ...
                        <template slot-scope="{ data }">
                            <vs-tr v-for="(tr, index) in data" :key="index" class="fila_lista">
                                ...
                                <vs-td class="nombrearreglo">{{ tr.nombre }}</vs-td>
                                <vs-td>
            ...
            <vs-divider position="left" class="flexy">
                <h3>Retenciones</h3>
                <vs-switch vs-icon-on="check" :disabled="respuesta_retencion_recupera == 'Enviado' " color="success" class="ml-2" v-model="retenciones.estado" @click="cambiosretenciones()" vs-value="Si" style="margin-top: 4px;">
                    <span slot="off">No</span>
                </vs-switch>
            </vs-divider>
            <transition name="slide-fade">
                <div class="vx-row leading-loose p-base" v-show="retenciones.estado">
                    <div class="w-full">
                        <p style="text-align: center;font-weight: bold;font-size: 18px;margin-bottom: 8px;" v-if="retencion_clave_acceso">Retención N° {{(retencion_clave_acceso).substring(24,27)}}-{{(retencion_clave_acceso).substring(27,30)}}-{{(retencion_clave_acceso).substring(30,39)}}</p>
                        <div class="vx-row hovertrash" v-for="(tr, index) in valorretenciones" :key="index">
                            <div class="w-2/3 ml-auto mr-auto">
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 text-center">
                                        ...
                                        <vs-select :disabled="respuesta_retencion_recupera == 'Enviado' " @change="recuperacion=true, agregarretencioniva(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.iva">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" class="w-full derecha" :disabled="respuesta_retencion_recupera == 'Enviado' " v-model="tr.baseiva" @click="recuperacion=true" @keyup="agregarretencionivavalor(index, tr.baseiva)"/>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                ...
                                <div class="vx-row">
                                    <div class="vx-col md:w-2/3 w-full mb-2 mr-auto text-center">
                                        ...
                                        <vs-select :disabled="respuesta_retencion_recupera == 'Enviado' " @change="recuperacion=true, agregarretencionrenta(index)" placeholder="Selecciona la retención" autocomplete class="selectExample w-full" v-model="tr.renta">
                                    ...
                                    <div class="flex-1 mb-2 mr-3 text-center">
                                        <vs-input label="Base" class="w-full derecha" :disabled="respuesta_retencion_recupera == 'Enviado' " v-model="tr.baserenta" @click="recuperacion=true" @keyup="agregarretencionrentavalor(index, tr.baserenta)"/>
                                    </div>
                                    <div class="flex-1 mb-2 mr-3 text-center">
                            ...
                            <feather-icon v-if="respuesta_retencion_recupera != 'Enviado' " icon="TrashIcon" style="position: absolute !important;right: 125px;margin-top: 80px;display: none;" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2 cursor-pointer trasher" @click="eliminararrayretencion(index)"/>
                            <feather-icon v-if="respuesta_retencion_recupera != 'Enviado' " @click="addretenciones()" icon="PlusIcon" style="position: absolute !important;right: 125px;margin-top: 55px;display: none;" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2 cursor-pointer trasher"/>
            ...
            <div class="vx-col w-full mt-5">
                <vs-button color="success" type="filled" :disabled="disabled_b" @click="pago_efectuado()">GUARDAR</vs-button>
                <vs-button color="danger" type="filled" to="/compras/liquidacion-compra">CANCELAR</vs-button>
            </div>
            <!-- Crear Cliente -->
...
export default {
    ...
    methods: {
        ...
        guardar_factura(){
            ...
            if(this.validar()){this.disabled_b = false;return;}

            if(this.total_pendiente>0){
                this.$vs.notify({
                    time: 8000,
                    title: "No se puede Guardar la liquidacion",
            ...
            if(this.pago_hecho=='si'){
                    this.$vs.notify({
                        title: "No se puede Editar el credito de la liquidacion",
                        text: "Ya se ha hecho un pago correspondiente a esta liquidacion",
                        color: "danger"
                    });
                    this.disabled_b = false;
                    return;
            }
            if(this.respuesta_retencion_recupera!=='Enviado' && this.retenciones.estado){
                ...
                axios.get(url).then(res => {
                    ...
                    this.retencion_clave_acceso = codigoacc + acceso;
                });
                //return false;
            }
            ...
            axios.put('/api/liquidacion_compra/editar_liquidacion', {
                ...
                retencion_clave_acceso:this.retencion_clave_acceso
                        ...
                        // if( ){
                            ...
                            // return;
                        // }
                        this.respuesta_retencion=data.retencion;
                        if(data.retencion=='Enviado' && data.liquidacion=="Enviado"){
                            this.$vs.notify({
                                time: 8000,
                                title: "Registro Actualizado",
                                text: "Este registro se actualizo exitosamente",
                                    ...
                                    if(this.retenciones.estado){
                                        if(data.retencion!=='Enviado'){
                                            if(this.valorretenciones[0].iva!=null || this.valorretenciones[0].renta!=null){
                                                ...
                                                var dataf = data.datos[0];
                                                this.recueidfact = data.datos[0].id_liquidacion_compra;
                                                axios.post('/api/factura/xml_compro_retenc',dataf).then(res => {
                                                    ...
                                                    var factura = DATA_EMPRESA + this.usuario.id_empresa + "/comprobantes/retencioncompra/" + this.factura.clave_acceso +".xml";
                        ...
                        if(data.liquidacion!=='Enviado'){
                                ...
                                axios.post(url, dataf).then(({data}) => {
                                        ...
                                        var fecha_actual = moment(dataf.fmodifica).format('LL');
        ...
        pago_efectuado(){
            this.pago_hecho="";
            axios.get("/api/liquidacion_compra/ctaxpagar/verficacion",{
                params:{
        ...
        //listar clave acceso
        listarclave() {
            if(this.respuesta_retencion_recupera == 'Enviado' || this.factura.observacion!==null){
        ...
        recuperardatos(){
            axios.get("/api/liquidacion_compra/recuperar/"+ this.$route.params.id).then( ({data}) => {
                console.log(data);
                ...
                this.factura = {
                    id_liquidacion_compra:data.factura.id_liquidacion_compra,
                ...
                this.retencion_clave_acceso=data.factura.observacion;
                ...
                this.anticipover(data.proveedor.id_proveedor);
                this.producto.tipo = true;
                data.detalle_factura.forEach(el => {
                    this.producto.lista_productos.push({
                        id_producto_bodega:el.id_producto_bodega,
                        id_detalle_liquidacion_compra:el.id_detalle_liquidacion_compra,
                ...
                if(data.iva.length>=1 || data.renta.length>=1){
                    ...
                    this.respuesta_retencion_recupera=data.factura.respuesta_retencion;
                }
        ...
        async crearfacturacion(firma, password, factura, tipo, usuario, id_factura, carpeta, fecha, valor, logo, nombre_empresa){
            try {
                ...
                if(!this.retenciones.estado) {
                    ...
                    if(this.respuesta_retencion!=='Enviado'){
                        if(this.valorretenciones[0].iva!=null || this.valorretenciones[0].renta!=null){
