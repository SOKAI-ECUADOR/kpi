...
use App\Models\ActaAgencia;
use Carbon\Carbon;


class ActaAgenciaController extends Controller
{
    
    //listar actaagencia
    public function indexactaagencia(Request $request, $id){
        $buscar = $request->buscar;
        $empresa = $id;
        $tipo = $request->tipo;
        if ($buscar == '') { 

            $recupera = DB::table('acta_agencia')
            ->join('provincia', 'provincia.id_provincia', '=', 'acta_agencia.provincia_id')
            ->join('ciudad', 'ciudad.id_ciudad', '=', 'acta_agencia.ciudad_id')
            ->join('parroquia', 'parroquia.id_parroquia', '=', 'acta_agencia.parroquia_id')
            ->where("acta_agencia.empresa_id", "=", $empresa)
            ->select(
                'acta_agencia.id as id',
                'acta_agencia.nombre as nombre',
                'acta_agencia.descripcion as descripcion',
                'provincia.nombre as provincia_nombre',
                'ciudad.nombre as canton_nombre',
                'parroquia.nombre_parroquia as nombre_parroquia'
                )
                ->orderByRaw('acta_agencia.nombre ASC')->get();             
            ...
            $recupera = DB::table('acta_agencia')
                ...
                ->where(function ($q) use ($buscar) {
                    $q->where('acta_agencia.nombre', 'like', '%'.$buscar.'%');
                })
                ->select(
                    'acta_agencia.id as id',
                    'acta_agencia.nombre as nombre',
                    'acta_agencia.descripcion descripcion',
                    'provincia.nombre as provincia_nombre',
                    'ciudad.nombre as canton_nombre',
                    'parroquia.nombre_parroquia as nombre_parroquia'
                    )
    ...
    //listar actaestados
    public function indexactaestado(Request $request){
        $buscar = $request->buscar;

        if ($buscar == '') { 
            $recupera = ActaEstado::select("*")
    ...
    public function storecolor(Request $request){
        $cod_color=DB::select("SELECT max(id_color) as id_color from color_mobiliario");
        $cod=1;
        if(count($cod_color)>0){
            $cod=$cod_color[0]->id_color+1;
        }
        $color=new ColorMobiliario();
        $color->id_color=$cod;
        $color->nombre_color=$request->nombre_color;
        $color->id_empresa=$request->id_empresa;
        $color->save();
    }
