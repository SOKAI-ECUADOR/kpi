...
ini_set('memory_limit', '4000M');
        ...
        if ($rol[0]->nombre_empresa == 'GRUPO SOLIS INGENIERIA ESPECIALIZADA' || $rol[0]->nombre_empresa == 'QUIMALCO CIA LTDA') {
            ...
            if($rol[0]->nombre_empresa == 'CORTINAS DLUXE'){
                ...
                if($rol[0]->nombre_empresa == 'ESPECIALIDADES MEDICAS Y DE FISIOTERAPIA ESMEDICFISIO S.A.'){
                    if ($rol[0]->nombre !== 'Usuario') {
                        if ($buscar == '') {
                            $recupera = DB::select("SELECT `factura`.*,factura.migo as migo_factura,factura.created_by as  created_by_factura,factura.updated_by as updated_by_factura,factura.id_vendedor as id_vendedor_factura, factura.estado as estadof, if(`factura`.`fecha_factura` is null,`factura`.`fmodifica`,`factura`.`fecha_factura`) as `fecha_autorizacion`, `empresa`.*, `cliente`.*, `moneda`.`nomb_moneda` as `moneda`, `factura`.`descuento` as `descuentototal`, `establecimiento`.`codigo` as `codigoes`, `punto_emision`.`codigo` as `codigope`, `establecimiento`.`direccion` as `direccion_establecimiento`, (SELECT id_guia FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS guia, (SELECT respuesta FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS respuesta_guia, (SELECT clave_acceso FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS clave_acceso_guia, (SELECT fecha_inicio_tr FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS fecha_inicio_tr 
                                                    from `factura` 
...
//if($request->datos["id_empresa"]==68){
//$recupera = DB::select("SELECT factura.*,factura.migo as migo_factura,factura.created_by as  created_by_factura,factura.updated_by as updated_by_factura,factura.id_vendedor as id_vendedor_factura, factura.estado as estadof, if(factura.fecha_factura is null,factura.fmodifica,factura.fecha_factura) as fecha_autorizacion, empresa.*, (select nombre from cliente c where c.id_cliente=cliente.id_cliente) as nombre, cliente.id_cliente as id_cliente, cliente.codigo as codigo, cliente.nombre_adicional as nombre_adicional, cliente.identificacion as identificacion, cliente.email as email, cliente.telefono as telefono, cliente.id_codigo_pais, moneda.nomb_moneda as moneda, factura.descuento as descuentototal, establecimiento.codigo as codigoes, punto_emision.codigo as codigope, establecimiento.direccion as direccion_establecimiento, (SELECT id_guia FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS guia, (SELECT respuesta FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS respuesta_guia, (SELECT clave_acceso FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS clave_acceso_guia, (SELECT fecha_inicio_tr FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS fecha_inicio_tr from factura inner join empresa on empresa.id_empresa = factura.id_empresa inner join cliente on cliente.id_cliente = factura.id_cliente inner join establecimiento on establecimiento.id_establecimiento = factura.id_establecimiento inner join punto_emision on punto_emision.id_punto_emision = factura.id_punto_emision inner join moneda on moneda.id_moneda = empresa.id_moneda where factura.id_empresa = {$request->datos["id_empresa"]} and factura.modo = 1 and factura.modo_acumulado is null and factura.id_punto_emision={$request->datos["id_punto_emision"]} ORDER BY factura.fecha_emision DESC,SUBSTR(factura.clave_acceso,25,15) desc");
//}
//else{
			            $recupera = DB::select("SELECT factura.*,factura.migo as migo_factura,factura.created_by as  created_by_factura,factura.updated_by as updated_by_factura,factura.id_vendedor as id_vendedor_factura, factura.estado as estadof, if(factura.fecha_factura is null,factura.fmodifica,factura.fecha_factura) as fecha_autorizacion, empresa.*, (select nombre from cliente c where c.id_cliente=cliente.id_cliente) as nombre, cliente.id_cliente, cliente.codigo, cliente.nombre_adicional, cliente.identificacion, cliente.direccion, cliente.email as email, cliente.telefono, cliente.contacto, cliente.estado, cliente.comentario, cliente.descuento, cliente.num_pago, cliente.id_codigo_pais, cliente.grupo_tributario, cliente.parte_relacionada, cliente.obligado_contabilidad, cliente.lista_precios, cliente.limite_credito, cliente.regimen_contribuyente, cliente.fcrea, cliente.fmodifica, cliente.umodifica, cliente.ucrea, cliente.id_forma_pagos, cliente.id_grupo_cliente, cliente.id_empresa, cliente.created_by, cliente.updated_by, cliente.id_tipo_cliente, cliente.id_plan_cuentas, cliente.id_provincia, cliente.id_cuidad, cliente.id_parroquia, cliente.id_seguro, cliente.id_plan_seguro, moneda.nomb_moneda as moneda, factura.descuento as descuentototal, establecimiento.codigo as codigoes, punto_emision.codigo as codigope, establecimiento.direccion as direccion_establecimiento, (SELECT id_guia FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS guia, (SELECT respuesta FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS respuesta_guia, (SELECT clave_acceso FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS clave_acceso_guia, (SELECT fecha_inicio_tr FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS fecha_inicio_tr from factura inner join empresa on empresa.id_empresa = factura.id_empresa inner join cliente on cliente.id_cliente = factura.id_cliente inner join establecimiento on establecimiento.id_establecimiento = factura.id_establecimiento inner join punto_emision on punto_emision.id_punto_emision = factura.id_punto_emision inner join moneda on moneda.id_moneda = empresa.id_moneda where factura.id_empresa = {$request->datos["id_empresa"]} and factura.modo = 1 and factura.modo_acumulado is null and factura.id_punto_emision={$request->datos["id_punto_emision"]} ORDER BY factura.fecha_emision DESC,SUBSTR(factura.clave_acceso,25,15) desc");
//}
		            } else {
                        $recupera = DB::select("SELECT `factura`.*,factura.migo as migo_factura,factura.created_by as  created_by_factura,factura.updated_by as updated_by_factura,factura.id_vendedor as id_vendedor_factura,factura.estado as estadof, if(`factura`.`fecha_factura` is null,`factura`.`fmodifica`,`factura`.`fecha_factura`) as `fecha_autorizacion`, `empresa`.*, `cliente`.*, `moneda`.`nomb_moneda` as `moneda`, `factura`.`descuento` as `descuentototal`, `establecimiento`.`codigo` as `codigoes`, `punto_emision`.`codigo` as `codigope`, `establecimiento`.`direccion` as `direccion_establecimiento`, (SELECT id_guia FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS guia, (SELECT respuesta FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS respuesta_guia, (SELECT clave_acceso FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS clave_acceso_guia, (SELECT fecha_inicio_tr FROM guia_remision WHERE id_factura = factura.id_factura LIMIT 1) AS fecha_inicio_tr 
                        from `factura` 
    ...
    public function ajustarfechafacturas(){
        ini_set('max_execution_time', 3000);
        ini_set('memory_limit','4000M');
        $datos = DB::select("SELECT * FROM factura WHERE fcrea is null AND fmodifica is null LIMIT 1000");

        for($i=0; $i<count($datos); $i++){

            DB::update("UPDATE factura set fcrea='".$datos[$i]->fecha_factura."', fmodifica='".$datos[$i]->fecha_factura."' where id_factura='".$datos[$i]->id_factura."'");
    ...
    public function ajustarfechafacturas2(){
        ini_set('max_execution_time', 3000);
        ini_set('memory_limit','4000M');
        $datos = DB::select("SELECT * FROM factura WHERE fcrea is null AND fmodifica is null LIMIT 1000");
        ...
        for($i=0; $i<count($datos); $i++){
            ...
            DB::update("UPDATE factura set fcrea='".$datos[$i]->fecha_emision." 00:00:00', fmodifica='".$datos[$i]->fecha_emision." 00:00:00' where id_factura='".$datos[$i]->id_factura."'");
    ...
    public function ajustarfechanotasdeventas(){
        ini_set('max_execution_time', 3000);
        ini_set('memory_limit','4000M');

        $datos = DB::select("SELECT * FROM nota_venta WHERE fcrea is null AND fmodifica is null LIMIT 1000");

        for($i=0; $i<count($datos); $i++){

            DB::update("UPDATE nota_venta set fcrea='".$datos[$i]->fecha_emision." 00:00:00', fmodifica='".$datos[$i]->fecha_emision." 00:00:00' where id_nota_venta='".$datos[$i]->id_nota_venta."'");
    ...
    public function ajustarfechabodegaegreso(){
        ini_set('max_execution_time', 3000);
        ini_set('memory_limit','4000M');
        $datos = DB::select("SELECT id_bodega_egreso, f.fcrea as fechaf FROM bodega_egreso be INNER JOIN factura f ON f.id_factura=be.id_factura WHERE be.fcrea IS NULL AND be.id_factura IS NOT NULL LIMIT 1000");

        for($i=0; $i<count($datos); $i++){

            DB::update("UPDATE bodega_egreso set fcrea='".$datos[$i]->fechaf."', fmodifica='".$datos[$i]->fechaf."' where id_bodega_egreso='".$datos[$i]->id_bodega_egreso."'");
    ...
    public function ajustarfechabodegaegreso2(){
        ini_set('max_execution_time', 3000);
        ini_set('memory_limit','4000M');
        $datos = DB::select("SELECT id_bodega_egreso, nv.fcrea as fechaf FROM bodega_egreso be INNER JOIN nota_venta nv ON nv.id_nota_venta=be.id_nota_venta WHERE be.fcrea IS NULL AND be.id_nota_venta IS NOT NULL LIMIT 1000");
    ...
    public function generaProforma(Request $request)
    {
        ...
        //dd($query);
        $proformas = DB::select("SELECT e.logo,e.nombre_empresa,e.ruc_empresa,e.direccion_empresa,e.email_facturacion, e.telefono as telefono_empresa,
        c.identificacion,f.fecha_emision,c.nombre as cliente,p.cod_principal,p.cod_alterno,df.p_descuento, 
        u.nombre as ciudad, c.contacto,c.direccion,c.email,c.telefono,p.imagen,p.nombre,p.descripcion,p.caracteristicas,p.normativa,p.uso,
        m.nombre as marca,tm.nombre as tipo_medida,um.nombre as unidad_medida,df.color,df.detalle as detalle_cortina,df.mando,df.alto,df.ancho,df.cantidad,df.precio,df.total as total_pro,df.tiempo_entrega,df.cpc,
        f.subtotal_12 as subtotal_12,f.subtotal_sin_impuesto as subtotal, f.descuento,f.codigo, f.iva_12 as iva, f.valor_total as total, CONCAT(v.nombres, ' ', v.apellidos) as vendedor,
